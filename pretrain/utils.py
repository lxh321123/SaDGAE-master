...
def degree_power(A, k):
    degrees = np.power(np.sum(A, axis=1), k)
    degrees[np.isinf(degrees)] = 0
    degrees = np.diag(degrees)
    return degrees

def normalize_adj(A):
    degrees = degree_power(A, k=-0.5)
    output = degrees.dot(A).dot(degrees)
    return output

def get_adj(x):
    A = kneighbors_graph(x.cpu().detach().numpy(), n_neighbors=args.neighbors, mode='connectivity', metric="cosine")
    A_nor = normalize_adj(A)
    A = A.toarray()
    A_nor = A_nor.toarray()
    return A, A_nor
...
def loss_cluster(z, c):
    q = 1.0 / (1.0 + torch.sum(torch.pow(z.unsqueeze(1) - c, 2), 2) / 1)
    q = q.pow((1 + 1.0) / 2.0)
    q = (q.t() / torch.sum(q, 1)).t()
    weight = q ** 2 / q.sum(0)
    p = (weight.t() / weight.sum(1)).t()
    return F.kl_div(q.log(), p, reduction='batchmean'), q
